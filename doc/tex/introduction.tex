...
In this use-case, the most painful operations where the integration would help
are:
...
is connected to a certain document. This is a different concept than the model
of folders and files, which is already familiar to a user.
\item A common pattern is to assign a task to a group of users, and then
requiring exactly one member of the group to perform the task.
...
\item In general, but especially when group tasks are involved, audit logs of
...
releasing the task back to the group, in the user realises she is not able to
perform it in time.
